...
def main():
    ...
    if sub == 3:
        idx, name, note = poses[0]  # Get the target pose
        
        print("\n  Waiting for checkpoint.json to exist (main.py must be running)...")
        while not os.path.exists("checkpoint.json"):
            time.sleep(0.4)
        ...
        while True:
            ...
            try:
                with open("checkpoint.json") as f:
                    ckpt = json.load(f)
                # Corrupt when we're at the pose BEFORE target
        ...
        try:
            ...
            # Alternative: kill all python processes running main.py
    ...
    for idx, name, note in poses:
        # ── FIX: Write signal BEFORE target pose is reached ──
        ...
        # Write the signal (main.py will detect it BEFORE moving to target)
        if sub == 4:
        ...
        while True:
            time.sleep(0.5)
            if not os.path.exists("checkpoint.json"):
                # Main.py cleared checkpoint (likely completed)
                break
            try:
                with open("checkpoint.json") as f:
                    ckpt = json.load(f)
